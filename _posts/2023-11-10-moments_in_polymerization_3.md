...
R(D_n)=&\overbrace{k_{t,d}\lambda_0 P_n}^{term\ by\ dispropotion}+\overbrace{k^{sol}_{tr}[S]P_n}^{sol\ trans}+\overbrace{k^{mon}_{tr}[M]P_n}^{mon\ trans}+\overbrace{{1\over 2}k_{t,c}\sum^{n-1}_{m=1}{P_m P_{n-m}}}^{term\ by\ combination}
\end{align}
$$