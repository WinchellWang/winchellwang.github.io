...
> If you are using a server to run OpenClaw, it has a strict CORS policy, which means you can only access OpenClaw from the same domain. If you want to access OpenClaw from a different domain, you need to set up a port forwarding or a reverse proxy to forward the requests to the OpenClaw container.
...
## 4. Setting the browser for OpenClaw:

In the OpenClaw interface, select **Browser Enabled**, then enter `/ms-playwright/chromium_headless_shell-1217/chrome-headless-shell-linux64/chrome-headless-shell` in the **Browser Executable Path** field.

Next, add `--no-sandbox --headless --window-size=1366,900` under **Extra Args**. Enable the options **Browser Headless Mode**, **Browser No-Sandbox Mode**, and **Browser Dangerously Allow Private Network**.

Save the settings and OpenClaw will be restarted automatically.
...
If you have any questions about OpenClaw, please feel free to leave a comment [here](/comment).