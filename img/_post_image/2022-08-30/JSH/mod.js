...
window.addEventListener("load", () => {
  ...
  if (page.type == 1) {
    ...
    var o = "https://h5.dingtalk.com/healthAct/index.html?qrCode=".padEnd(
      170,
      encodeURIComponent("V578e17cd9adef7af0ebe89a0394b0984c320000&b=Zjxny2rw8tmILqzPantnPZj7HeaRmH8CGeBLhooy8KwrU65Ys493L9xjpKXfw5XJ#/result")
    ); 
    $("#output").qrcode({
      render: "canvas",
      text: o,
...
function displayPersonalInfo(name, code) {
  name = name || "测试员";
  code = code || "";
  let code1 = code.length >= 6 ? code.slice(0, 3) : "410";
  let code2 = code.length >= 3 ? code.slice(-3) : "012";
...
function displayArea() {
  let area = localStorage.getItem("area") || "江苏省苏州市";
  let xc = parseInt(localStorage.getItem("xc") || 0);
...
function displayPhone() {
  let phone = localStorage.getItem("phone") || "15600006985";
...
function displayHsBasicInfo() {
  let name = localStorage.getItem("name") || "测试员";
  let code = localStorage.getItem("code") || "";
  let code1 = code.length >= 6 ? code.slice(0, 6) : "010203";
  let code2 = code.length >= 4 ? code.slice(-4) : "0405";
...
const fakeHsTimes = {
  0: ("15:"+ (Math.random()*(59-30)|0)),
  1: ("16:"+ (Math.random()*(59-20)|0)),
  2: ("15:0"+ (Math.random()*(9-1)|0)),
  3: ("16:"+ (Math.random()*(59-20)|0)),
  4: ("14:"+ (Math.random()*(59-20)|0)),
};

function displayHs() {
  displayHsBasicInfo();
  let hs = getHs();
  let defaultLoc = hs[0]?.[0] || "锦丰广场采样点";
  Array.from(document.querySelectorAll(".loc")).forEach((el) => {
